...
public class AgendamentoController {
    ...
    @GetMapping("/meus")
    @PreAuthorize("hasAuthority('CLIENTE')")
    public ResponseEntity<List<AgendamentoResponseDTO>> meusAgendamentos(@AuthenticationPrincipal Usuario clienteLogado) {
        List<AgendamentoResponseDTO> agendamentos = agendamentoService.verMeusAgendamentos(clienteLogado.getId());
        return ResponseEntity.ok(agendamentos);
    }

    @GetMapping("/todos")
    @PreAuthorize("hasAuthority('BARBEIRO')")
    public ResponseEntity<List<AgendamentoResponseDTO>> todosAgendamentos() {
        List<AgendamentoResponseDTO> agendamentos = agendamentoService.verTodosAgendamentos();
        return ResponseEntity.ok(agendamentos);
    }
